...
import { notifyStoreUsersCampaignEnrollment } from '../../services/customer-notifications';
...
export const campaignsModule = new Elysia({
      ...
      if (query.status) {
        conditions.push(
          eq(
            ...
            query.status as 'visible' | 'invited' | 'enrolled' | 'declined' | 'removed' | 'suspended',
          ),
        );
      }
      if (cursorDate) {
  ...
  .post(
    ...
    async ({
      ...
      const shouldNotifyEnrollment = targetStatus === 'enrolled' && existing?.status !== 'enrolled';
      ...
      const notificationSummary = shouldNotifyEnrollment
        ? await notifyStoreUsersCampaignEnrollment({
            campaignId: params.campaignId,
            storeId: params.storeId,
          })
        : { sent: 0, skipped: 0, failed: 0 };

      return { data: { success: true, status: targetStatus, notifications: notificationSummary } };
    },
    {
    ...
    async ({
      ...
      const serialized = await Promise.all(
        items.map(async (item) => {
          ...
          if (item.storeAccessMode === 'all_related_stores' && item.storeEnrollmentMode === 'auto_enroll') {
