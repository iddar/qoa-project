...
const parseCardLookupInput = (value: string): string | null => {
...
export default function StoreScanPage() {
    ...
    const createTransaction = useMutation({
        mutationFn: async () => {
            const customerLookup = parseCardLookupInput(cardInput);
            if (!customerLookup) {
            ...
            const customerResponse = await api.v1.stores[activeStoreId]["customer-resolve"].post(
            ...
            if (
                ...
                !customerResponse.data?.data.userId ||
                !customerResponse.data.data.cardId
            ) {
        ...
        onError: (error) => {
            if (error instanceof Error) {
                switch (error.message) {
                    case "CARD_REQUIRED":
                        setFeedback("Ingresa un cardId, código de tarjeta o payload QR válido.");
                        return;
                    case "STORE_REQUIRED":
        ...
        <div className="space-y-6">
            <header>
                ...
                <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                    Pega un cardId, código de tarjeta o payload QR JSON y genera una transacción en
                    segundos.
                </p>
            </header>
