...
export default function WalletHomePage() {
    ...
    const campaigns = (
        (walletQuery.data?.data.campaigns as WalletCampaign[] | undefined) ?? []
    ).filter((entry) => entry.enrollmentMode !== "system_universal");
    const storeBreakdown =
        (walletQuery.data?.data.storeBreakdown as WalletStoreBreakdown[] | undefined) ?? [];
    ...
    const tierState = walletQuery.data?.data.card.tierState as
        | "unqualified"
        | "qualified"
        | "at_risk"
        | undefined;
        ...
        <div className="space-y-4">
            <section className="relative overflow-hidden rounded-3xl border border-amber-300/70 bg-gradient-to-br from-amber-200 via-amber-100 to-amber-300 p-5 shadow-sm dark:border-amber-800/60 dark:from-zinc-900 dark:via-zinc-900 dark:to-zinc-800">
                ...
                <div className="relative z-10">
                    <div>
                        <p className="text-xs uppercase tracking-[0.18em] text-amber-800 dark:text-amber-300">
                            Tarjeta de lealtad
                        </p>
                        <p className="mt-2 text-6xl font-extrabold leading-none text-zinc-900 dark:text-zinc-100">
                            {walletQuery.data?.data.totals.current ?? 0}
                        </p>
                        ...
                        <p className="mt-6 text-sm text-zinc-700 dark:text-zinc-300">
                            Estado: <b>{walletQuery.data?.data.card.status ?? "active"}</b>
                        </p>
                        <p className="mt-1 text-sm text-zinc-700 dark:text-zinc-300">
                            Nivel:{" "}
                            <b>
                                {currentTier
                                    ? `${currentTier.order} · ${currentTier.name}`
                                    : "Base"}
                            </b>
                        ...
                        <p className="mt-1 text-sm text-zinc-700 dark:text-zinc-300">
                            Alta:{" "}
                            {walletQuery.data?.data.card.createdAt
                                ? new Date(walletQuery.data.data.card.createdAt).toLocaleDateString(
                                      "es-MX",
                                  )
                                : "--"}
                        </p>
                        ...
                        {currentTier && (
                            <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-300">
                                Ventana de mantenimiento: {currentTier.windowValue}{" "}
                                {currentTier.windowUnit}(s)
                            </p>
                        )}
                ...
                >
                    <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl dark:bg-zinc-900">
                        ...
                        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                            Muéstrala en caja para registrar tus compras.
                        </p>
            ...
            {nextGoal && (
                <section className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900/70">
                    ...
                    <p className="mt-3 text-sm text-zinc-700 dark:text-zinc-300">
                        {nextGoal.name} ({nextGoal.progress}%) - ¡Casi la tienes!
                    </p>
                    ...
                    <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
                        Te faltan {nextGoal.missing} puntos para canjear.
                    </p>
                </section>
            )}
            ...
            {storeBreakdown.length > 0 && (
                <section className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900/70">
                        ...
                        {storeBreakdown.slice(0, 5).map((store) => (
                            <li key={store.storeId} className="py-3 first:pt-0 last:pb-0">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <p className="shrink-0 text-sm font-bold text-amber-600 dark:text-amber-300">
                                        {store.pointsTotal} pts
                                    </p>
                                </div>
                    ...
                    <Link
                        href="/transactions"
                        className="mt-3 inline-block text-sm font-semibold text-zinc-900 dark:text-zinc-100"
                    >
